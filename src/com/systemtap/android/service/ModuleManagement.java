...
public class ModuleManagement extends Observable
{
	...
	public ModuleManagement()
	{
		mModuleDir = new FileObserver(Config.MODULES_ABSOLUTE_PATH,FileObserver.MOVED_FROM|FileObserver.MOVED_TO|FileObserver.CREATE|FileObserver.DELETE)
		{
			...
			{
				Log.d(ModuleManagement.TAG, "Something changed in " + Config.MODULES_ABSOLUTE_PATH + "(" + path + "), event=" + event);
	...
	public synchronized Module createModule(String pName)
	{
		...
		{
			Log.e(TAG, "Try to insert existing module");
		}
		return module;
	}
	...
	public synchronized void updateModuleStatus(String pName, ModuleStatus pStatus)
	{
		...
		{
			Log.e(TAG, "Try to update non-existing module: " + pName);
		}
	}
	...
	public void save() {
		for (Module module : mModules.values()) {
			...
			try {
				...
				Log.e(TAG,"save(): Can't save modules meta information: " + moduleConfFile.getAbsolutePath());
			}
		}
	}
	...
	private synchronized void updateModules(boolean pLoadConf) 
	{
		...
		Log.d(TAG, "Updating module list...");
		...
		Log.d(TAG, "Got file list from " + Config.MODULES_ABSOLUTE_PATH + ", testing for new modules");
		for(File moduleFile: moduleFiles)
		{
			String filename = moduleFile.getName();
			String modulename = filename.substring(0, filename.indexOf(Config.MODULE_EXT));
			Log.d(TAG, "Does " + modulename + " exist in database?");
			Module module = mModules.get(modulename);
			if (module == null)
			{
				Log.d(TAG,"No, create a new module (" + modulename + ")");
			...
			else
			{
				...
				Log.d(TAG,"Yes, doing nothing");
			}
			if (pLoadConf) {
				Log.d(TAG,"Reading module conf file...");
				File moduleConfFile = new File(Config.MODULES_ABSOLUTE_PATH + File.separator + module.getName() + Config.MODULE_CONF_FILE_EXT);
				try {
					...
					Log.e(TAG,"updateModules(): Can't parse module config: " + moduleConfFile.getAbsolutePath());
				}
			}
		}
		
		Log.d(TAG,"Checking if all modules in database exist");
		for(Module module:mModules.values())
		{
			File moduleFile = new File(Config.MODULES_ABSOLUTE_PATH + File.separator + module.getName() + Config.MODULE_EXT);
			Log.d(TAG, "Does module " + module.getName() + " exist in module directory?");
			if (!moduleFile.exists())
			{
				if (module.getStatus() == ModuleStatus.RUNNING)
				{
					Log.d(TAG, "No, it does not exist, but it will not be deleted because it is currently running.");
				}
				else
				{
